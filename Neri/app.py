...
from db_wrapper import get_connection, IntegrityError
...
def run_schema():
    try:
        db = get_connection(app.config['DATABASE'])
...
def _safe_migrations():
    ...
    try:
        ...
        # Ensure daily_finance exists (since new installs might fail on executing full schema.sql if tables already exist)
        try:
            db.execute('''
            ...
            print(f"Finance table migration note: {e}")
...
@login_required
def finance():
    """Finance page for tracking daily spending."""
    today = datetime.date.today().isoformat()
    return render_template('finance.html', today=today)

@app.route('/api/finance/data', methods=['GET'])
@login_required
def get_finance_data():
    """Get finance data for a specific date or all dates for charts."""
    ...
    if date:
        entries = db.execute(
            'SELECT * FROM daily_finance WHERE user_id = ? AND entry_date = ? ORDER BY created_at DESC', 
            (uid, date)
        ).fetchall()
        ...
        # Aggregated data for charts (last 30 days)
        thirty_days_ago = (datetime.date.today() - datetime.timedelta(days=30)).isoformat()
        entries = db.execute(
            'SELECT * FROM daily_finance WHERE user_id = ? AND entry_date >= ? ORDER BY entry_date ASC', 
            (uid, thirty_days_ago)
        ).fetchall()
...
def add_finance_entry():
    ...
    try:
        db.execute(
            'INSERT INTO daily_finance (user_id, entry_date, category, amount, notes) VALUES (?, ?, ?, ?, ?)',
            (uid, data['date'], data['category'], float(data['amount']), data.get('notes', ''))
        )
        db.commit()
...
@app.route('/api/finance/delete', methods=['POST'])
@login_required
def delete_finance_entry():
    ...
    db.execute('DELETE FROM daily_finance WHERE id = ? AND user_id = ?', (data['id'], uid))
    db.commit()
    return jsonify({'status': 'success'})
...
def get_date_view():
    ...
    finance_entries = db.execute('SELECT amount FROM daily_finance WHERE user_id = ? AND entry_date = ?', (uid, date_str)).fetchall()
    finance_total = sum(e['amount'] for e in finance_entries)
...
def init_physical_activities():
    ...
    try:
        default_activities = [
            ...
            ('Walk 4000 steps', 'Cardio', 'Walk 4000 steps', 45),
            ('Gym Workout - Upper Body', 'Strength', 'Weight lift shoulder/chest exercises', 60),
            ('Gym Workout - Lower Body', 'Strength', 'Leg press, squats, deadlifts', 60),
            ('Gym Workout - Full Body', 'Strength', 'Full body gym session', 60),
        ]
...
def _generate_career_goals(profile):
    ...
    if ptype == 'Student':
        ...
        goals += random.sample([
            ...
            'Watch 1 mock interview video on YouTube',
            'Review a successful resume online and note 2 good ideas',
            'Draft a cold email/message for a potential mentor',
            'Explore 1 potential career path (read a day-in-the-life blog)'
        ], 2)
        if 'web' in field or 'frontend' in field or 'ui' in field or 'ux' in field:
            goals += random.sample([
                ...
                'Learn 1 advanced CSS trick (e.g., clamp, subgrid)',
                'Optimize a past project for Lighthouse score > 90',
                'Implement dark mode in a mini-project'
            ], 2)
        elif 'ai' in field or 'ml' in field or 'machine' in field or 'data' in field:
            goals += random.sample([
                ...
                'Watch a tutorial on a deep learning framework (PyTorch/TF)',
                'Find and clean a dirty dataset for training',
                'Experiment with a HuggingFace pre-trained model'
            ], 2)
        elif 'mobile' in field or 'android' in field or 'ios' in field or 'flutter' in field:
            goals += random.sample([
                'Publish or update 1 app on a store',
                'Implement 1 new screen in your side project',
                'Add local storage/SQLite to a simple app',
                'Build a custom animation for a screen transition',
                'Debug memory leaks in a past mobile project'
            ], 2)
            ...
            goals += random.sample([
                ...
                'Set up a local vulnerable VM (Metasploitable)',
                'Read 1 recent bug bounty writeup',
                'Practice writing a secure login script'
            ], 2)
            ...
            goals += random.sample([
                ...
                'Add proper error handling and logging to an API',
                'Set up Docker for a local development project',
                'Implement JWT authentication for a sample backend'
            ], 2)
        else:
            goals += random.sample([
                'Learn 1 new technical skill or tool this week',
                'Document learnings — write a blog or notes',
                'Take a 30-min crash course on a new technology'
            ], 2)
        ...
        goals += random.sample([
            ...
            'Prepare a "brag document" for performance reviews',
            'Identify one mentor or sponsor at your company',
            'Attend or register for 1 industry webinar/event'
        ], 2)
        if any(k in role for k in ('engineer', 'developer', 'dev', 'software')):
            goals += random.sample([
                ...
                'Profile code and find one performance bottleneck',
                'Draft a tech spec for a proposed feature'
            ], 2)
            ...
            goals += random.sample([
                ...
                'Optimize a slow SQL query in your codebase',
                'Set up data quality alerting for 1 pipeline'
            ], 2)
            ...
            goals += random.sample([
                ...
                'Map out the stakeholder matrix for a project',
                'Have a 1:1 specifically focused on career growth'
            ], 2)
        else:
            goals += random.sample([
        ...
        goals += random.sample([
            ...
            'Update your media kit or portfolio',
            'Create a batch of 5 evergreen content ideas'
        ], 4)
        ...
        goals += random.sample([
            ...
            'Read 1 book chapter related to personal growth'
        ], 4)
